"""
Sovereign Brain LLM Client Wrapper
Replaces OpenAI API calls with native C++ Sovereign inference
"""
...
# Define the path to the Sovereign DLL/SO in the backend root
# Final Production Path (Root of the infrastructure)
LIB_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "sovereign.dll"))
...
class SovereignEngine:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.lib.sovereign_init_fragment.argtypes = [ctypes.c_char_p]
            self.lib.sovereign_init_fragment.restype = ctypes.c_void_p
            self.lib.sovereign_agent_set_fragment.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
            
            self.lib.sovereign_agent_save_state.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
            self.lib.sovereign_agent_load_state.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
            ...
            self.lib.sovereign_free_agent.argtypes = [ctypes.c_void_p]
            self.lib.sovereign_load_compact.argtypes = [ctypes.c_void_p, ctypes.c_char_p]
            self.lib.sovereign_save_compact.argtypes = [ctypes.c_void_p, ctypes.c_char_p]
            
            # Neural Injection (Phase 7: Distillation)
            self.lib.sovereign_train_step_distill.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.POINTER(ctypes.c_float), ctypes.c_float]
            self.lib.sovereign_train_distill_bulk.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_int), ctypes.c_int, ctypes.c_float]

            # Fragmentation Persistence
            self.lib.sovereign_get_fragment_bias.restype = ctypes.POINTER(ctypes.c_float)
            
            # Tokenization (Phase 7.2: High-Fidelity Mapping)
            self.lib.sovereign_tokenize.argtypes = [ctypes.c_char_p]
            self.lib.sovereign_tokenize.restype = ctypes.c_int
            self.lib.sovereign_detokenize.argtypes = [ctypes.c_int]
            self.lib.sovereign_detokenize.restype = ctypes.c_char_p
            
            # Allocation
            self.master_brain = self.lib.sovereign_init_master()
            if os.path.exists("master_brain_compact.bin"):
                self.lib.sovereign_load_compact(self.master_brain, b"master_brain_compact.bin")
                print(f"[SOVEREIGN] Hive Engine Initialized (16MB Compact).")
        except Exception as e:
            print(f"[SOVEREIGN ERROR] Hive Engine failed: {e}")
            self.lib = None

class SovereignSwarm:
    """The Swarm Director: Orchestrates collaboration between agent fragments"""
    def __init__(self):
        self.engine = SovereignEngine.get_instance()
        self.agent_ptr = self.engine.lib.sovereign_init_agent(b"SwarmDirector", self.engine.master_brain, 42)
        self.specialists = {}

    def spawn_specialist(self, name: str, bias_data: Optional[np.ndarray] = None):
        """Register a new specialist fragment in the swarm"""
        frag_ptr = self.engine.lib.sovereign_init_fragment(name.encode('utf-8'))
        ...
        if os.path.exists(soul_file):
            ...
            # Restore bias
            bias_ptr = self.engine.lib.sovereign_get_fragment_bias(frag_ptr)
    ...
    def collaborate(self, task: str, sequence: List[str]) -> Dict[str, str]:
        ...
        for name in sequence:
            ...
            self.engine.lib.sovereign_agent_set_fragment(self.agent_ptr, self.specialists[name])
            
            # Observe and Act
            prompt = f"Relay from previous agent: {current_input}\nTask: {task}"
            self.engine.lib.sovereign_agent_observe(self.agent_ptr, prompt.encode('utf-8'))
            
            raw_res = self.engine.lib.sovereign_agent_act(self.agent_ptr, 100, 0.7)
...
class LLMClient:
    ...
    def chat(self, messages: List[Dict[str, str]], personality: str = "Generalist", **kwargs) -> str:
        ...
        self.engine.lib.sovereign_agent_set_fragment(self.swarm.agent_ptr, self.swarm.specialists[personality])
        
        context = "\n".join([f"[{m['role'].upper()}]: {m['content']}" for m in messages])
        self.engine.lib.sovereign_agent_observe(self.swarm.agent_ptr, context.encode('utf-8'))
        
        raw_res = self.engine.lib.sovereign_agent_act(self.swarm.agent_ptr, 200, 0.7)
    ...
    def train_step(self, input_token: int, teacher_probs: np.ndarray, lr: float = 0.001):
        ...
        self.engine.lib.sovereign_train_step_distill(self.swarm.agent_ptr, input_token, probs_ptr, lr)
    ...
    def train_bulk(self, tokens: List[int], lr: float = 0.001):
        ...
        self.engine.lib.sovereign_train_distill_bulk(self.swarm.agent_ptr, tokens_arr, n, lr)
